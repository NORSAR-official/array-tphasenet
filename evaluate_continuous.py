...
from continuous_prediction_utils import (
    load_manual_picks,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog='Example: python evaluate_continuous.py --config config.yaml'
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    print('Config read.')
    png_search_dirs = [cfg.run.outputdir, cfg_pred.output_dir]
    ...
    if not cfg_pred.combine_array_stations and not cfg_pred.combine_beams :
            ...
            if stat != 'X' and stat in cfg_pred.stations :
                ...
                if cfg.evaluation.optimal_threshold :
                    ...
                    thr_opt_p,thr_opt_s,fig = plot_L_curve(p_true, p_pred, s_true, s_pred,
                                           model, cfg, True, stat , useforprec=(ap_true,as_true),eval_output_dir='./')
                    fig.show()
                else :
        ...
        if cfg.evaluation.optimal_threshold :
            ...
            thr_opt_p,thr_opt_s,fig = plot_L_curve(p_true, p_pred, s_true, s_pred,
                                       model, cfg, True, suf , useforprec=(ap_true,as_true),eval_output_dir='./')
            #fig.show()
        else :
            thr_opt_p=cfg.evaluation.p_threshold
            thr_opt_s=cfg.evaluation.s_threshold
    ...
    report_produced_pngs(png_before, png_search_dirs, show_images=cmd_args.show_produced_png)
